    ...
    <div className="mt-8 rounded-xl bg-gradient-to-br from-yellow-50 to-amber-50 border border-yellow-200 p-6 text-center">
      ...
      {!analysisGenerated && (
        <div className="mt-6 border-t border-yellow-200 pt-5">
          ...
          <p className="text-sm text-yellow-700 mb-4">
            Pour cela, <strong>créez votre compte</strong> après avoir généré votre analyse individuelle.
          </p>
          {!showRegister ? (
            <button
              onClick={() => setShowRegister(true)}
              className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm px-6 py-3 shadow-md transition-colors cursor-pointer"
            >
              🔓 Créer mon compte
            </button>
          ) : (
            <div className="mt-2 rounded-xl bg-white border border-indigo-200 p-5 text-left shadow-sm">
              <h3 className="text-sm font-bold text-indigo-800 mb-2 text-center">
                Créer mon compte pour accéder aux analyses collectives
              </h3>
              <div className="flex items-start gap-2 mb-4">
                ...
                />
                <label htmlFor="rgpd-pre" className="text-xs text-gray-600 cursor-pointer">
                  En créant mon compte, j'accepte que mes résultats d'égogramme (scores, profil et analyses générées) soient sauvegardés et associés à mon compte afin de permettre les analyses collectives. Ces données ne seront jamais partagées avec des tiers sans mon consentement.
                </label>
              </div>
