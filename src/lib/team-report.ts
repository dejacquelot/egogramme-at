...
function stripMd(s: string) {
  ...
  return expandEgoStateAbbreviations(s)
    ...
    .replace(/\*\*/g, "")
    .replace(/\*([^*\n]+)\*/g, "$1")
    .replace(/_([^_\n]+)_/g, (whole, inner: string, offset: number, full: string) => {
      const before = offset > 0 ? full.charAt(offset - 1) : " ";
      const after = full.charAt(offset + whole.length) || " ";
      return /[A-Za-z0-9]/.test(before) || /[A-Za-z0-9]/.test(after) ? whole : inner;
    })
    .replace(/`/g, "")
    .trim();
}
...
function analysisToBlocks(analysis: string): Block[] {
  ...
  while (i < lines.length) {
    ...
    // Fenced code block: on retire uniquement les délimiteurs et on continue à
    // interpréter le contenu normalement (l'IA ne doit pas produire de bloc de code).
    if (isFence(lines[i])) {
      i++;
      continue;
    }
    ...
    if (line === ":::pae") {
      ...
      i++; // skip :::
      const groups = parsePaeBlock(pLines.join("\n"));
      if (groups) out.push({ type: "pae", groups });
      continue;
    }
    ...
    if (line === ":::karpman") {
      ...
      const get = (role: string) => {
        const found = kLines.find((l) => norm(l.trim()).startsWith(norm(role) + ":"));
        return found ? stripMd(found.slice(found.indexOf(":") + 1).trim()) : role;
      };
      out.push({
        type: "karpman",
        persecuteur: get("Persécuteur"),
        sauveur: get("Sauveur"),
        victime: get("Victime"),
      });
      continue;
    ...
    // sinon un schéma dessiné à la main serait avalé comme un tableau.
    if (isPipeLine(lines[i]) && !isSepLine(lines[i])) {
      ...
      if (probe < lines.length && isSepLine(lines[probe])) {
        ...
        while (i < lines.length) {
          ...
          } else if (isHr(lines[i]) && i + 1 < lines.length && (isPipeLine(lines[i + 1]) || isSepLine(lines[i + 1]))) {
            i++; // skip horizontal rule between table rows (sometimes generated by AI)
          } else {
            break;
          }
        }
    ...
    // Horizontal rule (---, ***, ___) → real separator instead of literal dashes
    if (isHr(line)) {
      out.push({ type: "rule" });
      i++;
      continue;
    ...
    if (/^>\s?/.test(line)) {
      ...
      while (i < lines.length && /^\s*>\s?/.test(lines[i])) {
        ...
        if (content.trim()) quoted.push(stripMd(content));
        i++;
      }
      if (quoted.length) out.push({ type: "quote", text: quoted.join(" ") });
      continue;
    }
...
function drawPae(ctx: CanvasRenderingContext2D, groups: PaeInstance[], x: number, y: number, w: number) {
  font(ctx, 11, "700");
  ctx.fillStyle = INK;
  ctx.textAlign = "left";
  ctx.fillText("⚖️ Répartition de l'énergie", x, y + 10);

  const gap = 10;
  const n = groups.length || 1;
  const cardW = (w - gap * (n - 1)) / n;
  const cardY = y + 20;
  const cardH = PAE_H - 26;
  ...
  groups.forEach((g, idx) => {
    ...
    ctx.fillText(g.label.toUpperCase(), cx + 10, cardY + 16);

    font(ctx, 17, "700");
    ctx.fillStyle = INK;
    const totalText = String(g.total);
    ctx.fillText(totalText, cx + 10, cardY + 36);
    const totalW = ctx.measureText(totalText).width;
    font(ctx, 9);
    ctx.fillStyle = MUTED;
    ctx.fillText(`/${g.max}`, cx + 10 + totalW + 2, cardY + 36);

    const barW = cardW - 20;
    const pct = g.max > 0 ? g.total / g.max : 0;
    ctx.fillStyle = "#e2e8f0";
    roundRect(ctx, cx + 10, cardY + 44, barW, 5, 2.5);
    ctx.fill();
    if (pct > 0) {
      ctx.fillStyle = g.color;
      roundRect(ctx, cx + 10, cardY + 44, Math.max(3, barW * pct), 5, 2.5);
      ctx.fill();
    }

    font(ctx, 8);
    ctx.fillStyle = MUTED;
    ctx.fillText(g.parts.map((p) => `${p.short} ${p.value}`).join(" · "), cx + 10, cardY + 62);
  });
}
...
function blockHeight(ctx: CanvasRenderingContext2D, b: Block, w: number): number {
  switch (b.type) {
    ...
    case "quote":
      font(ctx, 11.5, "italic 400");
      return wrap(ctx, b.text, w - 18).length * 17 + 12;
    case "li":
      font(ctx, 11.5);
...
function drawBlock(ctx: CanvasRenderingContext2D, b: Block, x: number, y: number, w: number) {
  switch (b.type) {
    ...
    case "quote": {
      font(ctx, 11.5, "italic 400");
      const qLines = wrap(ctx, b.text, w - 18);
      ctx.strokeStyle = NAVY;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(x + 1, y + 2);
      ctx.lineTo(x + 1, y + qLines.length * 17 + 6);
      ctx.stroke();
      ctx.fillStyle = MUTED;
      qLines.forEach((l, i) => ctx.fillText(l, x + 14, y + 13 + i * 17));
      return;
    }
    case "li": {
      font(ctx, 11.5);
